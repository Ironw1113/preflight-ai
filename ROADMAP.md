...
- [x] File-upload task estimator (Single task tab): upload the file you want the task run on and `avgInputWords` is set from the file's real word count instead of a guess. Server extracts text (`server/lib/fileExtract.js`, files processed in memory only, never persisted) from plain text/code, PDF, DOCX, and XLSX/XLS; a bounded snippet of the content also feeds `classifyTask()` for better task-type detection on both the Claude and heuristic paths.
