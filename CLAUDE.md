...
- `client/` — React + Vite, single `src/App.jsx` with four mode tabs (Single task / Multi-step workflow / Code estimator / Approvals), dark theme in `src/styles.css`. `ApprovalRequestForm` renders on every results view and posts to `/api/approvals`; `ApprovalsMode` is the audit-trail list with approve/reject actions. Dev server proxies `/api` to :3001.
